...
- [x] Scoreboard panel layout refined so live team scoring occupies the main top row, game controls and game status sit in a full-width secondary row, accrued sets/points are shown in `Game Status`, and scoreboard team headers display `Team Name (Team A/B)`.
